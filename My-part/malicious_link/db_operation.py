...
def get_domains():
	...
	# conn.commit()
	print 'update over ...'
	# SQL = "select domain_index.domain from domain_index, malicious_info where domain_index.ID = malicious_info.ID and malicious_info.flag not like '%2_' and malicious_info.flag not like '%1_' or malicious_info.flag < 10;"
	# SQL = "select domain_index.domain from domain_index, malicious_info where domain_index.ID = malicious_info.ID and malicious_info.flag < 10 limit 100;"
	SQL = "SELECT domain from domain_index where judge_flag < 0 and judge_flag > -10 and ID in (select ID from malicious_info where flag like '%0_' or flag <10)"
	cur.execute(SQL)
	result = cur.fetchall()
	for item in result:
		domain_q.put(item[0])
