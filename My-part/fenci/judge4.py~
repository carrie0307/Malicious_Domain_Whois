...
def get_domains(conn, cur):
    global domain_q
    SQL = "SELECT domain from domain_index where ID not in (select ID from malicious_info) LIMIT 8000"
    cur.execute(SQL)
    result = cur.fetchall()
    for item in result:
        domain_q.put(item[0])
...
def commit_result(conn, cur):
    ...
    while True:
        try:
            domain, title, keywords, malicious_keywords, grade, flag = res_q.get(timeout=300)
        except Queue.Empty:
            break
