...
# 选择时候的flag数值  (现在代表类型)
def get_domains(conn, cur):
	global domain_title_q
	global keywors_q
	# conn = MySQLdb.connect('172.26.253.3', 'root', 'platform', 'malicious_domain_sys', charset = 'utf8')
	# cur = conn.cursor()
	# SQL = "SELECT domain, malicious_info.title FROM malicious_info, domain_index WHERE malicious_info.ID = domain_index.ID AND flag < 10000 limit 1000"
	SQL = "SELECT domain, malicious_info.title FROM malicious_info, domain_index WHERE domain_index.ID = malicious_info.ID and domain_index.judge_flag < 0 and domain_index.judge_flag > -10 AND malicious_info.flag < 10000"
	cur.execute(SQL)
	result = cur.fetchall()
	for item in result:
...
def Baidu_get_raw_html(domain, title):
	...
	for item in wd_list:
		...
		try:
			ini_html = requests.get('http://www.baidu.com/s',params=ini_payload, headers=headers, timeout=5).text
			# print ini_html
...
def Bing_get_raw_html(domain, title):
	...
	for item in wd_list:
		...
		try:
			...
			whether_exist = '没有找到与 <strong>' + item + '</strong> 相关的结果'
			if whether_exist in ini_html:
			 	# print whether_exist
				counts.append(0)
				continue
			else:
...
def get_domain_influ():
	...
	while True:
		...
		if Bing_index > -1 or Baidu_index > -1: # 只要二者一个存在结果
			count = max(Baidu_index, Bing_index)
			print "count: " + str(count)
			date = time.strftime('%Y-%m-%d', time.localtime(time.time()))
			print domain, count, date
			res_q.put([domain, int(count), date])
	print 'get domain influ ends ...'




def commit_result(conn, cur):
	# global conn, cur
	global res_q
	global counter
	while True:
		try:
			domain, count, date = res_q.get(timeout=100)
		except Queue.Empty:
			break
		try:
...
def run():
	...
	for td in get_influ_td:
		td.start()
 	time.sleep(10)
	save_db_td = threading.Thread(target=commit_result, args=(conn, cur))
	save_db_td.start()
	save_db_td.join()

if __name__ == '__main__':
	run()
	# Baidu_get_raw_html('gdffsdfssdfsfsd', '')
	# get_domains(1)
